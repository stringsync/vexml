...
/**
 * Opens corpus scores in a real browser: screenshot() for pixels, probe() for a value
 * computed against the live Score (with the pixels riding along). Encapsulates the
 * repo's test conventions — the __data__ corpus, the Docker system fonts, the
 * reference-width viewport, the #screenshot crop — over a pool of tabs (see pool.ts),
 * so a test states only what it is about. setup.ts constructs the run's one shared
 * instance (`scores`) in beforeAll and closes it in afterAll; importing a module never
 * constructs anything.
 */
export class ScoreBrowser {
	constructor(private readonly pool = new TabPool()) {}

	/** Warm the pool: bundle the page, launch the browser, open the first tab. */
	start(): Promise<void> {
