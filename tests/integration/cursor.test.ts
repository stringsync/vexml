...
test('grace notes resolve to targets with real geometry off their host onsets', async () => {
	const browser = await testBrowser();
	const page = await browser.newPage({ viewport: { width: 900, height: 400 } });
	try {
		await page.goto(TEST_URL);
...
test('tab grace notes resolve to fret targets with real geometry', async () => {
	const browser = await testBrowser();
	const page = await browser.newPage({ viewport: { width: 900, height: 400 } });
	try {
		await page.goto(TEST_URL);
