...
test('right/bottom derive from x/y/w/h', () => {
	const r = new Rect(10, 20, 30, 40);
	expect(r.right).toBe(40);
	expect(r.bottom).toBe(60);
});

test('intersects is strict — overlapping rects hit, edge-touching does not', () => {
...
test('intersection returns the overlap rect or null', () => {
	const a = new Rect(0, 0, 10, 10);
...
test('contains is inclusive of edges', () => {
...
test('translate returns a shifted copy without mutating the original', () => {
...
test('fromBoundingBox reads the vexflow accessors', () => {
