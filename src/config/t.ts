...
export class t {
  static defaultConfig<T extends AnySchema>(schema: T): SchemaConfig<T> {
    const value = <T extends SchemaDescriptor>(descriptor: T) => {
      switch (descriptor.type) {
        ...
        case 'debug':
          return value(descriptor.child as SchemaDescriptor);
      }
    };
  ...
  static enum<T extends [string, ...string[]]>(choices: T, defaultValue = choices[0]) {
