...
test('query matches a brute-force oracle, including out-of-bounds items', () => {
...
test('items outside the root bounds land in the outside bucket and are still queryable', () => {
	...
	const hits = new Set(tree.query(new Rect(-60, -60, 30, 30)).map((i) => i.id));
	expect(hits).toEqual(new Set([2]));
});

test('clear resets the tree and the outside bucket', () => {
